 ...
 * dequeue the priority queue, get an array that contains all string 
 * elements, free a name list, clear the queue, get the priority of an element,
 * remove elements between input range of priorities, and change the priority
 * of an element.
 */
...
/* free all the dynamically-allocated memory of the strings in name_list. 
   we can assume that the last element in name_list is always NULL if name_list
   is not NULL. it should do nothing if the array name_list is NULL*/
void free_name_list(char *name_list[]) {
  int count = 0;
  
  if (name_list != NULL) {
    while (name_list[count] != NULL)
...
/* deallocate all dynamically-allocated memory that is used in prio_q.
   It will have no effect if prio_q is NULL. */
...
/* return the priority of the target element of prio_q. 
   It will return the highest priority if there are more than one target 
   element in prio_q. 
   It will have no effect if the target element doesn't exist in prio_q. */
int get_priority(Prio_que prio_q, char element[]) {
  ...
  if (element != NULL) {
    ...
    /* find the first occurance of element */
    while (curr != NULL && strcmp(curr->data, element) != 0 )
      curr = curr->next; 
    
    if (curr != NULL && strcmp(curr->data, element) == 0)
...
/* remove all elements from prio_q whose priority is greater than or equal to
   low, and less than or equal to high, and then return the number of elements
   removed from the queue. It will return 0 if nothing is removed. */
unsigned int remove_elements_between(Prio_que *const prio_q, unsigned int low,
  ...
  /* skip all the elements that has higher priority than high */
  while (curr != NULL && curr->priority > high) {
    prev = curr;
    curr = curr->next;
  }
  
  /* remove the elements that has higher or equal priority than low */
  while (curr != NULL && curr->priority >= low) {
    free(curr->data);
    prev->next = curr->next; /* make previous node points to next next node */
    free(curr);
    curr = prev->next;
    removed++;
  }
...
/* change the priority of the element in prio_q to new_priority and return 1.
   It should have no effect and return 0 if element is not in prio_q, or
   if there is already an element with priority new_priority, or there are 
   more than one occurance of element in prio_q with different priorities. */
unsigned int change_priority(Prio_que *prio_q, char element[],
  ...
  /* return 0 if new_priority already exists and count the number of 
     the occurance of the target element */
  while (curr != NULL) {
